...
CSpriteSequence::CSpriteSequence( CAnimEditorDoc* pDoc, CSpriteCatalog* pCat, CSequence* pSeqToCopy, BOOL bRLE )
{
	...
	iMoveSpeed	= pSeqToCopy->iMoveSpeed;
...
*/
	...
	while( posInList )
	{
		...
		if (pSeqCel->bActionFrame)
			iActionFrame = iFrameCount;

		int index = IndexOfCel( pDoc, pSeqCel->pCel );
		arIndexes.SetAt( iFrameCount, index );
/*
		if( !bRLE )
		{
...
//-----------------------------------------------------------------------------------------------------------------
#ifdef ANIMEDITORDOC_VERSION
int CSpriteSequence::IndexOfCel( CAnimEditorDoc* pDoc, CCel* pCelFind )
{
	//	Returns index of where cel would be in the array of utilized cels.
	//	( Broken out just to keep the constructor clean. )
